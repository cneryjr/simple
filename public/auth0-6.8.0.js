...
function Auth0 (options) {
  ...
  this._useJSONP = null != options.forceJSONP ?
                    !!options.forceJSONP :
                    use_jsonp() && !same_origin('http:', options.domain);
...
Auth0.prototype.parseHash = function (hash) {
  ...
  // iss should be the Auth0 domain (i.e.: http://contoso.auth0.com/)
  if (prof.iss && prof.iss !== 'http://' + this._domain + '/') {
    return invalidJwt(
      'The domain configured (http://' + this._domain + '/) does not match with the domain set in the token (' + prof.iss + ').');
  }
...
Auth0.prototype._authorize = function(options) {
  ...
  var url = joinUrl('http:', this._domain, '/authorize?' + query);
...
Auth0.prototype.loginPhonegap = function (options, callback) {
  ...
  var mobileCallbackURL = joinUrl('http:', this._domain, '/mobile');
  var _this = this;
  var qs = [
    this._getMode(options),
    options,
  ...
  var popupUrl = joinUrl('http:', this._domain, '/authorize?' + query);
...
Auth0.prototype.loginWithPopup = function(options, callback) {
  ...
  var popup = WinChan.open({
    url: popupUrl,
    relay_url: 'http://' + this._domain + '/relay.html',
...
Auth0.prototype.loginWithUsernamePasswordAndSSO = function (options, callback) {
  ...
  var popup = WinChan.open({
    url: 'http://' + this._domain + '/sso_dbconnection_popup/' + this._clientID,
    relay_url: 'http://' + this._domain + '/relay.html',
    window_features: stringifyPopupSettings(popupOptions),
...
Auth0.prototype._verify_redirect = function(options) {
  ...
  var query = this._buildAuthorizeQueryString(qs);
  var url = joinUrl('http:', this._domain, '/passwordless/verify_redirect?' + query);
 ...
 *
 *     auth0.logout();
 *     // redirects to -> 'http://yourapp.auth0.com/logout'
 *
 * @example
 *
 *     auth0.logout({returnTo: 'http://logout'});
 *     // redirects to -> 'http://yourapp.auth0.com/logout?returnTo=http://logout'
 *
 * @method logout
 * @param {Object} query
 */
...
Auth0.prototype.logout = function (query) {
  var url = joinUrl('http:', this._domain, '/logout');
  if (query) {
...
Auth0.prototype.getSSOData = function (withActiveDirectories, callback) {
  ...
  var url = joinUrl('http:', this._domain, '/user/ssodata');
...
Auth0.prototype.getConnections = function (callback) {
  return jsonp('http://' + this._domain + '/public/api/' + this._clientID + '/connections', jsonpOpts, callback);
};
 ...
 * source: http://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/isArray
 *
 * @param {Array} array
 * @public
 */
...
function use_jsonp() {
  ...
  // if ('XDomainRequest' in window && window.location.protocol === 'http:') {
  //   return false;
  // }
...
;(function () {
  ...
  // encoder
  // [http://gist.github.com/999166] by [http://github.com/nignag]
  object.btoa || (
  ...
  // decoder
  // [http://gist.github.com/1020396] by [http://github.com/atk]
  object.atob || (
...
!function (name, context, definition) {
  ...
  function succeed(r) {
    ...
    return httpRe.test(protocol) ? twoHundo.test(r.request.status) : !!r.request.response;
  }
...
var WinChan = (function() {
  ...
  // given a URL, extract the origin. Taken from: http://github.com/firebase/firebase-simple-login/blob/d2cb95b9f812d8488bdbfba51c3a7c153ba1a074/js/src/simple-login/transports/WinChan.js#L25-L30
  function extractOrigin(url) {
    if (!/^http?:\/\//.test(url)) url = window.location.href;
    var m = /^(http?:\/\/[\-_a-zA-Z\.0-9:]+)/.exec(url);
    if (m) return m[1];
    return url;
  }
        ...
        function cleanup() {
          ...
          if (w) {
            try {
              ...
              // see http://github.com/mozilla/browserid/issues/1844
...
(function () {
	...
	// modified from http://github.com/kriskowal/es5-shim
